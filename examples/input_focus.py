...
async def ppytty_task():
    ...
    log_win = await api.window_create(0, 0, 1.0, 8, dx=2, dy=1, dw=-4, bg=31)

    async def log(msg):
        log_win.print(msg + '\r\n')
        await api.window_render(log_win)

    p1_win = await api.window_create(0, 10, 0.5, 1.0, dx=2, dw=-3, dh=-11, bg=0)
    p1_win.title = '/bin/bash'
    p1 = await api.process_spawn(p1_win, ['/bin/bash'])
    await log(f'Process spawned: {p1!r}')

    p2_win = await api.window_create(0.5, 10, 0.5, 1.0, dx=1, dw=-3, dh=-11, bg=4)
